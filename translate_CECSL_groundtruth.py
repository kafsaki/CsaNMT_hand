...
if __name__ == '__main__':
    json_file_path = 'CE-CSL-translated-groundtruth.json'  # JSON 文件路径
    ...
    # 提取所有 'groundtruth' 字段，存入一个列表（跳过已经翻译过的项）
    groundtruth_list = [entry['groundtruth'] for entry in json_data[skipped_count:] if entry.get('groundtruth')]
    ...
    for batch_num in range(total_batches):
        ...
        # 打印当前批次的信息
        print(f"\n\033[32mProcessing batch {batch_num + 1}/{total_batches} (Groundtruth {start_index + 1} to {end_index})\033[0m")
